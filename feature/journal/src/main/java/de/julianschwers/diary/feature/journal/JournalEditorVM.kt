...
class JournalEditorVM(
    ...
    init {
        ...
        if (journalUid == null) {
            ...
            viewModelScope.launch { journal.emit(newJournal) }
            
            Log.i(tag, "JournalEditorVM initialised using newly created Journal (${newJournal.uid}).")
        } else {
            viewModelScope.launch(Dispatchers.IO) {
                val foundJournal = repository.getJournal(journalUid)
